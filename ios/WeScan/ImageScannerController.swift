...
public final class ImageScannerController: UINavigationController {
    ...
    public required init() {
        ...
        self.delegate = self
        self.imageScannerDelegate = self
...
extension ImageScannerController:ImageScannerControllerDelegate, UINavigationControllerDelegate {
    public func imageScannerController(_ scanner: ImageScannerController, didFinishScanningWithResults results: ImageScannerResults) {
        ...
        guard let imagdata = UIImageJPEGRepresentation(results.scannedImage, 1.0),//UIImagePNGRepresentation(results.scannedImage),
            let writeUrl = SIFileManager.fileUrlForDocumentNamed("Image.jpg") else {
                postURl("", "Try Again")
                return
        }
        
        do {
            try imagdata.write(to: writeUrl)
            //            let result = self.openURL123(url: URL(string: "deftFax://com.myapp.share")!)
            
        } catch let error {
            print("error copying file : ")
            print(error.localizedDescription)
        }
        postURl(writeUrl.absoluteString, "")
    }
    
    public func imageScannerControllerDidCancel(_ scanner: ImageScannerController) {
        print("imageScannerControllerDidCancel scanner")
        postURl("", "imageScannerControllerDidCancel")
    }
    
    public func imageScannerController(_ scanner: ImageScannerController, didFailWithError error: Error) {
        print("imageScannerControllerDidCancel didFailWithError")
        postURl("", "didFailWithError")
    }
    
    func postURl(_ strUrl:String = "", _ error:String = "") {
        self.dismiss(animated: true) {
            if !error.isEmpty {
                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "WEScanImagePost"), object: self, userInfo: ["error":error])
                return
            }
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "WEScanImagePost"), object: self, userInfo: ["url":strUrl])
        }
    }
}

let appGroupIdentifier = "group.deftfax.com"
...
class SIFileManager
{
    ...
    static func fileUrlForDocumentNamed(_ name: String) -> URL? {
        ...
        var protectedName = "\(Date().timeIntervalSince1970)"
        protectedName = protectedName.replacingOccurrences(of: ".", with: "")
        if name.isEmpty {
    ...
    static func appGroupContainerURL() -> URL? {
        ...
        let storagePathUrl = groupURL//.appendingPathComponent("Storage")
